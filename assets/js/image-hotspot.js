(function ($, _) {
  ...
  const initImageHotspot = ($scope) => {
    ...
    const onHotspotTriggerEvent = (event) => {
      ...
      const isHotspotButton = $target.closest(
      ...
      const isMobile =
        window.elementorFrontend.getCurrentDeviceMode() === "mobile";
      const isLink = $target.closest(".deensimc-image-hotspot--link").length;

      const shouldTriggerTooltip = !(
        isLink &&
        isMobile &&
        (event.type === "mouseleave" || event.type === "mouseenter")
      );
      ...
      if (shouldTriggerTooltip && (isHotspotButton || isTooltipMouseLeave)) {
        ...
        $hotspot
          .not($currentHotspot)
          .removeClass("deensimc-image-hotspot--active");
    ...
    const setTooltipPosition = () => {
      ...
      const isDirectionalAnimation =
        animation &&
        animation.match(/^deensimc-image-hotspot--(slide|fade)-direction/);
      ...
      if (isDirectionalAnimation) {
        ...
        $tooltip.addClass(
          `deensimc-image-hotspot--tooltip-animation-from-${position}`
        );
      }
    };
    ...
    const runSequencedAnimation = () => {
      const isSequenced =
        settings.deensimc_hotspot_sequenced_animation === "yes";
        ...
        callback: (event) => {
          if (event.isInViewport) {
            ...
            $hotspot.each((index, element) => {
              ...
              const duration =
                settings.deensimc_hotspot_sequenced_animation_duration?.size ||
                1000;
